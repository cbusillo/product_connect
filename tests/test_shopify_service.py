...
class ShopifyClient:
    def __init__(
        self, http_client: object | None = None, url: str | None = None
    ) -> None:
        pass
...
class DummySyncRecord:
    id: int = 1
    hard_throttle_count: int = 0


class ShopifyServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        self.service: ShopifyService = ShopifyService(DummyEnv(), DummySyncRecord())

    def test_compute_throttle_delay_none_when_no_data(self) -> None:
        self.assertIsNone(self.service._compute_throttle_delay({}))

    def test_compute_throttle_delay_zero_when_sufficient_points(self) -> None:
        data = {
            "extensions": {
                "cost": {
                    "throttleStatus": {
                        "currentlyAvailable": self.service.MIN_API_POINTS
                    }
                }
            }
        }
    ...
    def test_compute_throttle_delay_calculates_delay(self) -> None:
        data = {
            "extensions": {
                "cost": {
    ...
    def test_throttle_info_detects_throttled_error(self) -> None:
        data = {
    ...
    def test_throttle_info_handles_non_throttled_error(self) -> None:
        data = {
            "errors": [{"message": "bad"}],
