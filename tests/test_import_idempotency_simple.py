...
class TestImportIdempotencySimple(ShopifyTestBase):
    ...
    def test_import_since_last_import_filters_correctly(self):
        ...
        old_order_data = create_shopify_order_response(
            ...
            customer=create_shopify_customer_response(
                gid="gid://shopify/Customer/old_cust",
                email="old@test.com"
            ),
            line_items=[
                create_shopify_order_line_item_response(
        ...
        new_order_data = create_shopify_order_response(
            ...
            customer=create_shopify_customer_response(
                gid="gid://shopify/Customer/new_cust",
                email="new@test.com"
            ),
            line_items=[
                create_shopify_order_line_item_response(
        ...
        def mock_fetch_page(client, query, cursor):
            ...
            if query and 'updated_at:>"' in query:
                ...
                if cursor is None:
                    mock_page.nodes = [
                        OrderFields(**old_order_data),
                        OrderFields(**new_order_data)
                    ]
                else:
                    mock_page.nodes = []
    ...
    def test_first_import_gets_all_data(self):
        ...
        order_data = create_shopify_order_response(
            ...
            name="#FIRST-001",
            customer=create_shopify_customer_response(
                gid="gid://shopify/Customer/first_cust",
