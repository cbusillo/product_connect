...
export class MultigraphModel extends GraphModel {
    ...
    async load(searchParams) {
        ...
        this._prepareMultiMeasureData();

        // Ensure dataPoints is populated for hasData() to work
        if (this.data && this.data.labels) {
            this.dataPoints = this.data.labels.map((label, index) => ({
                labels: [label],
                value: this.data.datasets[0]?.data[index] || 0,
                count: 1
            }));
        }
